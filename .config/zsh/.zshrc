...
alias proj='PROJDIR=$(find ~/Maja/Projects -maxdepth 1 -type d | fzf) && cd "$PROJDIR" && PROJFILES=$(find "$PROJDIR" -maxdepth 2 -type f -printf "%T@ %p\n" | sort -n | cut -f2- -d" " | xargs file | grep "text" | cut -f1 -d":" | tail -n5) && [[ -n "$PROJFILES" ]] && vim "$PROJFILES" || printf "Directory is now %s, but no text files here.\n" "$PROJDIR" && ls'
